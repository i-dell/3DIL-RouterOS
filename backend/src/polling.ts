...
export class PollingService extends EventEmitter {
 private driver:HuaweiDriver; private config=getRouterRuntimeConfig(); private timer:ReturnType<typeof setTimeout>|null=null; private snapshot:RouterSnapshot|null=null; private polling=false; private running=false; private backoff=this.config.pollIntervalMs; private previous=new Set<string>(); private signature=''; private lastError:string|null=null; private logs:Array<{timestamp:string;level:'info'|'warning'|'error';message:string}>=[];
 constructor(driver=new HuaweiDriver()){super();this.driver=driver}
 async start(username:string,password:string){if(this.running)this.stop();this.running=true;try{await this.driver.connect({username,password});this.log('info','Router login verified');await this.refresh();this.schedule(this.config.pollIntervalMs)}catch(e){this.log('error','Router login failed');throw e}}
 ...
 getAuthDiagnostics(){return this.driver.getAuthDiagnostics()}
 ...
 private log(level:'info'|'warning'|'error',message:string){this.logs.push({timestamp:new Date().toISOString(),level,message});if(this.logs.length>500)this.logs.shift()}
 private detectDevices(s:RouterSnapshot){const ids=new Set(s.devices.items.map(d=>d.mac?.toUpperCase().replace(/[^0-9A-F]/g,'')).filter((x):x is string=>!!x));for(const x of ids)if(!this.previous.has(x)){this.log('info','Device connected');this.emit('deviceConnected',s,x)}for(const x of this.previous)if(!ids.has(x)){this.log('info','Device disconnected');this.emit('deviceDisconnected',s,x)}this.previous=ids}
}
