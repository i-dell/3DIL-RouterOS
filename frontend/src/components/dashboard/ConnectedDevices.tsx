  ...
  return (
    <section className="rounded-[28px] border border-emerald-800/40 bg-[#07120d] p-4 sm:p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="rounded-full border border-emerald-800/50 px-3 py-1 text-sm text-emerald-300">تحديث</button>
      </div>
          ...
          {devices.map((device) => (
            <div key={device.id} className="grid grid-cols-[1.2fr_0.9fr_1.1fr_0.7fr_0.7fr_0.6fr] items-center gap-3 px-3 py-3 text-sm text-[#dcefe0]">
              <div>
                ...
                <p className="text-xs text-[#6e8772]">{device.status}</p>
              </div>
              <div className="text-xs text-[#8fb29d]">{device.ip}</div>
              <div className="text-xs font-mono text-[#8fb29d]">{device.mac}</div>
              <div className="text-xs text-[#8fb29d]">{device.band}</div>
              <div className="text-xs text-[#8fb29d]">{device.signal}</div>
              <button className="rounded-full border border-emerald-800/40 px-2 py-1 text-[11px] text-emerald-300">{device.actionLabel}</button>
            </div>
          ))}
        </div>
      </div>
